  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-xl font-bold" style={{ color: 'var(--text-primary)' }}>Order Details</h1>
          <p className="text-xs font-mono mt-0.5" style={{ color: 'var(--text-muted)' }}>#{id.slice(0, 8).toUpperCase()}</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4">
          {/* Customer info card */}
          <div className="dark-card p-5 shadow-card">
            <h2 className="text-sm font-semibold mb-4 uppercase tracking-wide" style={{ color: 'var(--text-secondary)' }}>Customer</h2>
          ...
          <div className="dark-card p-5 shadow-card">
            <h2 className="text-sm font-semibold mb-3 uppercase tracking-wide" style={{ color: 'var(--text-secondary)' }}>Delivery Address</h2>
          ...
          {/* Order items card */}
          <div className="dark-card p-5 shadow-card">
            <h2 className="text-sm font-semibold mb-3 uppercase tracking-wide" style={{ color: 'var(--text-secondary)' }}>Items Ordered</h2>
            {items.length === 0 ? (
              <p className="text-sm" style={{ color: 'var(--text-muted)' }}>No items recorded</p>
            ) : (
              <ul className="space-y-2">
                {items.map((item, idx) => (
                  <li key={idx} className="flex items-center justify-between text-sm">
                    <span style={{ color: 'var(--text-secondary)' }}>{item.name || item.title}</span>
                    <span className="font-medium" style={{ color: 'var(--text-muted)' }}>×{item.quantity}</span>
                  </li>
                ))}
              </ul>
            )}
          ...
          {order?.status !== 'confirmed' && order?.status !== 'cancelled' && (
            <div className="dark-card p-5 shadow-card">
              <h2 className="text-sm font-semibold mb-3 uppercase tracking-wide" style={{ color: 'var(--text-secondary)' }}>Manual Override</h2>
              <p className="text-xs mb-4" style={{ color: 'var(--text-muted)' }}>Manually update this order's status if the customer responded outside WhatsApp.</p>
              <div className="grid grid-cols-2 gap-2">
                <button
                  id="override-confirm-btn"
                  onClick={() => setModalAction('confirmed')}
                  className="py-2.5 bg-green-600 hover:bg-green-700 text-white text-sm font-semibold rounded-xl transition active:scale-95 flex items-center justify-center gap-1.5 shadow-lg shadow-green-900/20"
                >
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                  </svg>
                ...
                <button
                  ...
                  className="py-2.5 bg-red-600 hover:bg-red-700 text-white text-sm font-semibold rounded-xl transition active:scale-95 flex items-center justify-center gap-1.5 shadow-lg shadow-red-900/20"
                >
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                  </svg>
        ...
        <div className="lg:col-span-2">
          <div className="dark-card shadow-card h-full flex flex-col">
            {/* Chat header */}
            <div 
            ...
            >
              ...
              <div>
                <p className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>WhatsApp Conversation</p>
...
function InfoRow({ label, value, mono }) {
  return (
    <div className="flex items-start justify-between gap-2">
      <span className="text-xs font-medium uppercase tracking-wide flex-shrink-0" style={{ color: 'var(--text-muted)' }}>{label}</span>
      <span 
